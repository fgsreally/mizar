...
function applyOptions(instance) {
  ...
  if (computedOptions) {
    for (const key in computedOptions) {
      ...
      const set2 = !isFunction(opt) && isFunction(opt.set)
...
const warnRuntimeUsage = method => warn2(`${method}() is a compiler-hint helper that is only usable inside <script setup> of a single file component. Its arguments should be compiled away and passing it at runtime has no effect.`)
...
const ssrContextKey = Symbol(true ? 'ssrContext' : '')
var useSSRContext = () => {
  {
