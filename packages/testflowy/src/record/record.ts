...
import type { TesterElement, TestflowyEvent } from './data'
import { sdkStorage, setLastRecord } from './data'
import type { EventMapKeys } from './eventMap'
...
async function getElementData(eventName: string, element: TesterElement, event: any): Promise<TestflowyEvent> {
  ...
  const out = {
    ...
    value: (element as unknown as { __mockvalue: string }).__mockvalue || value || undefined,
